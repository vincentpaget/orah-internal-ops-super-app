...
export default function RenewalsDueSoonTable({ opps, activeTile }: Props) {
  const sorted = [...opps].sort((a, b) => {
    ...
    // Sort by Renewal Date ascending (earliest first), nulls last
    const aDate = a.Renewal_Date_1__c ?? ''
    const bDate = b.Renewal_Date_1__c ?? ''
  ...
  return (
    <div style={{ overflowX: 'auto', position: 'relative' }}>
      <table style={{ width: '100%', minWidth: 3470, borderCollapse: 'separate', borderSpacing: 0 }}>
        <thead>
          <tr>
            ...
            <th style={{ ...TH, width: 110, textAlign: 'center' }}>Notice Period Waived</th>
              ...
              <tr key={opp.Id}>
                ...
                <td style={{ ...TD, width: 110 }}>
                  {opp.Renewal_Date_1__c
                    ? <DatePill date={opp.Renewal_Date_1__c} noWarning={isClosed} />
                    : <span style={{ color: 'var(--fg-3)' }}>—</span>}
                </td>
                <td style={{ ...TD, width: 100 }}>
                  <span style={{ fontSize: 13, color: 'var(--fg-2)', whiteSpace: 'nowrap' }}>{shortDate(opp.CloseDate)}</span>
                </td>
                <td style={{ ...TD, width: 110, textAlign: 'center' }}>
                  <BoolCell value={opp.Notice_Period_Waived__c} />
                </td>
