...
export default function SAOHealthTable({ opps }: Props) {
  ...
  if (failing.length === 0) {
    return (
      <div style={{
        ...
        All SAO opportunities are clean — no flags.
      </div>
    )
  }
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', minWidth: 1700, borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed' }}>
        <thead>
          <tr>
            ...
            <th style={{ ...TH, ...COL.freeText }}>Nurturing Reason</th>
            <th style={{ ...TH, ...COL.date }}>Re-Eng Date</th>
            <th style={{ ...TH, ...COL.grade }}>CE</th>
            <th style={{ ...TH, ...COL.freeText }}>CE Reason</th>
          ...
          {failing.map(opp => (
            <tr key={opp.Id} style={{ transition: 'background 120ms' }}>
              ...
              <td style={{ ...TD, color: 'var(--fg-2)', ...COL.freeText }}>
                <TruncatedText
                  text={opp.Nurturing_Reason__c}
                  fallback={<span style={{ color: 'var(--red-600)', fontWeight: 600 }}>Not set</span>}
                />
              </td>
              <td style={{ ...TD }}>
                {opp.Re_engagement_Date__c
                  ? <DatePill date={opp.Re_engagement_Date__c} />
                  : <span style={{ color: 'var(--red-600)', fontWeight: 600, ...FS.body }}>Not set</span>}
              </td>
              <td style={{ ...TD }}>
